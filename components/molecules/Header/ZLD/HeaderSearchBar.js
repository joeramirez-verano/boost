  ...
  const Hit = ({hit}) => (
    <>
      {hit.post_type == 'location' && (
        <Link href={hit.slug ? '/locations/' + hit.slug : '#'} className="block py-2 text-sm text-black hover:underline">
          {hit.post_title.replace('�', 'Ü')}
        </Link>
      )}

      {hit.post_type == 'post' && (
        <Link href={hit.slug ? '/post/' + hit.slug : '#'} className="block py-2 text-sm text-black hover:underline">
          {hit.post_title.replace('�', 'Ü')}
        </Link>
      )}
