...
export default function DeliveryStoreInfoCard({deliveryStore}) {
  ...
  if (deliveryStore.type === 'hub') {
    ...
    if (sweedStore) storeInfo = getSweedStoreDeliveryInfo(sweedStore)
    else storeInfo = getJaneStoreDeliveryInfo(deliveryStore)
  }
            ...
            <button
              ...
              onClick={(e) => {
                ...
                if (storeInfo.type === 'jane') {
                  ...
                  router.push(`/locations/${storeInfo?.slug}`)
                  return
                } else if (storeInfo.type === 'hub') {
                  storeSwitcherController.toggle()
                  router.push(storeInfo.url)
                  return
                } else if (isDesktop) {
