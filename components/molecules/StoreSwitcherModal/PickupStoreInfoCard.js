 ...
 * StoreInfoCard
 * @param            props0
 * @param            props0.storeLocation
 * @param            props0.variant
 * @return {Element}                      - The StoreInfoCard component.
 */
    ...
    <>
      <div
        ...
        onClick={() => {
          ...
          // conditionally close the all stores modal if for mobile version
          isAllStoresModalOpen && storeSwitcherController.toggleAllStoresModal()
        }}
      >
        <div className="flex w-full flex-col items-start gap-4 rounded-[4px]">
          <div className="flex w-full items-center justify-around gap-4">
            <div className="flex w-full flex-1 flex-col items-start">
              ...
              <span className="text-xs">
                {address1}
                {address2 ? ' ' + address2 : ''}, {storeLocation?.nap?.city}, {storeLocation?.state?.abbr?.toUpperCase()} {storeLocation?.nap?.zip}, USA
              </span>
            </div>
            ...
            <button
              ...
              onClick={(e) => {
                ...
                storeSwitcherController.selectPickupStore(storeLocation)
                if (isDesktop) {
                  // Handle Jane menu stores here: redirect to storeInfo page of jane menu store
                  if (!sweedStoreDetails) {
                    storeSwitcherController.toggle()
                    router.push(`/locations/${storeLocation?.slug}`)
                    return
                  } else {
          ...
          <div>
            <div className="text-xs">Today: {isStoreClosed ? 'Closed' : `${todayStoreHours?.open} - ${todayStoreHours?.closed}`}</div>
