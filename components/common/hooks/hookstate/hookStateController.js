...
export const hookStateController = (state, initialState) => ({
  ...
  update: (partialState) => state.merge(partialState), // partially updates the state
  partialUpdateKey: (key, obj) => {
    const prevState = state[key].get({noproxy: true})
    hookStateController.update({[key]: {...prevState, ...obj}})
  },
  ...
  getValues: (keys) => {
    ...
    const keysValue = {}
    keys.forEach((key) => {
      Object.defineProperty(keysValue, key, {
        get() {
          return state[key].get({noproxy: true})
        }
      })
    })
    return keysValue
