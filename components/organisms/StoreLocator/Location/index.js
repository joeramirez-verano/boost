      ...
      >
        <div className="grid h-full grid-cols-5 gap-4">
          ...
          <div className="col-span-3 flex h-full flex-col justify-between">
            <Link href={store.location_status !== 'Open' ? getStoreInfoUrl(store) : storeMenuUrl} className="leading-0 flex justify-between">
            ...
            {!hideButtons && (
              <div className="flex gap-3 pb-0 pt-4 lg:gap-4 [&>*:first-child]:!px-1">
                <GeoStateFlag geoState={GEOSTATE.USA}>
                  {store.location_status !== 'Open' ? (
                    <Button className="!mb-0 flex-auto !text-base" size="sm" tag="div">
                      <Link href={getStoreInfoUrl(store)} aria-label={'View Store Info for ' + store?.post_title || store?.title}>
                        <>
                          View Store Info
                ...
                {store.location_status !== 'Coming Soon' ? (
                  <Link href={`${getStoreInfoUrl(store)}`} className={`has-tooltip group flex items-center ${fixWidth ? 'w-64' : ''}`}>
                    <span
